...
import { ArrowLeft, BarChart3, BookOpen, Brain, Check, CheckCircle2, ChevronRight, Clock3, Flame, GraduationCap, Home, Library, ListChecks, Menu, NotebookPen, Play, Search, Sparkles, Target, Trophy, X } from 'lucide-react'
...
function App(){
  ...
  const completeLesson=()=>{
    ...
    const p={...progress,completed:[...progress.completed,key],xp:progress.xp+50,minutes:progress.minutes+lesson.duration,lastSubject:subject.id,lastLesson:lesson.id}
...
function TopBar({screen,go,menuOpen,setMenuOpen}:{screen:Screen;go:(s:Screen)=>void;menuOpen:boolean;setMenuOpen:(v:boolean)=>void}){
  ...
  return <header className="topbar"><div className="topbar-inner">
    <button className="brand" onClick={()=>go('home')}><span className="brand-mark">S</span><span><strong>Study Hub</strong><small>личная академия</small></span></button>
    ...
    {!root&&<div className="location-pill">Учебный материал</div>}
  </div></header>
}
...
function HomeScreen({progress,percent,openSubject,openLesson,go}:{progress:Progress;percent:number;openSubject:(s:Subject)=>void;openLesson:(s:Subject,l:Lesson)=>void;go:(s:Screen)=>void}){
  ...
  const institute=subjects.filter(s=>!['military','attention','logic','ai'].includes(s.id))
  return <div className="page home-page">
    <section className="welcome"><div><p className="overline">1 семестр · личная программа</p><h1>Доброе утро, Екатерина!</h1><p>Учимся не ради галочки — собираем знания, которые останутся с тобой надолго.</p></div><div className="avatar">ЕК</div></section>
    <section className="announcement"><Sparkles size={20}/><div><strong>Большое обновление платформы</strong><span>Теперь предмет → темы → полноценная лекция → практика → тест → конспект.</span></div></section>
    <section className="hero-learning">
      <div className="hero-copy"><span className="eyebrow">Продолжить обучение</span><h2>{lastS.title}</h2><p>{lastL.title}</p><div className="hero-meta"><span><Clock3 size={15}/>{lastL.duration} мин</span><span><BookOpen size={15}/>лекция + тест</span></div><button onClick={()=>openLesson(lastS,lastL)}><Play size={17}/>Продолжить</button></div>
      <div className="hero-score"><div className="ring" style={{'--p':`${percent*3.6}deg`} as React.CSSProperties}><strong>{percent}%</strong><span>всего курса</span></div><div className="mini-stats"><span><Flame size={16}/>{progress.streak} день</span><span><Trophy size={16}/>{progress.xp} XP</span></div></div>
    </section>
    <SectionTitle title="Предметы семестра" action="Все предметы" onClick={()=>go('learning')}/>
    <div className="semester-table"><div className="table-head"><span>Дисциплина</span><span>Контроль</span><span></span></div>{institute.slice(0,7).map(s=><SubjectRow key={s.id} subject={s} progress={progress} onClick={()=>openSubject(s)}/>)}</div>
    <SectionTitle title="Дополнительная подготовка"/>
    <div className="special-grid">{subjects.filter(s=>['ai','military','attention','logic'].includes(s.id)).map(s=><button key={s.id} className={`special-card ${s.tone}`} onClick={()=>openSubject(s)}><span>{s.icon}</span><div><strong>{s.title}</strong><small>{s.description}</small></div><ChevronRight size={18}/></button>)}</div>
  </div>
}
...
function LearningScreen({progress,filtered,query,setQuery,openSubject}:{progress:Progress;filtered:Subject[];query:string;setQuery:(v:string)=>void;openSubject:(s:Subject)=>void}){
  return <div className="page"><div className="page-title"><div><p className="overline">Учебный план</p><h1>Обучение</h1><p>Все предметы первого семестра и дополнительные направления.</p></div><GraduationCap size={40}/></div>
...
function SubjectRow({subject,progress,onClick,showProgress=false}:{subject:Subject;progress:Progress;onClick:()=>void;showProgress?:boolean}){
  ...
  const p=Math.round(done/subject.lessons.length*100)
  return <button className="subject-row" onClick={onClick}><span className={`status-dot ${done?'started':''}`}></span><span className="subject-name"><b>{subject.icon}</b><strong>{subject.title}</strong></span><span className="control">{subject.control}</span><span className="row-progress">{showProgress?<><i><em style={{width:`${p}%`}}/></i><small>{done}/{subject.lessons.length}</small></>:<ChevronRight size={18}/>}</span></button>
}
...
function SubjectScreen({subject,progress,back,openLesson}:{subject:Subject;progress:Progress;back:()=>void;openLesson:(s:Subject,l:Lesson)=>void}){
  ...
  return <div className="page narrow"><button className="back" onClick={back}><ArrowLeft size={18}/>Все предметы</button>
    <section className={`subject-hero ${subject.tone}`}><div className="subject-icon">{subject.icon}</div><div><span>{subject.control} · семестр {subject.semester}</span><h1>{subject.title}</h1><p>{subject.description}</p><div className="subject-progress"><i><em style={{width:`${p}%`}}/></i><small>{done} из {subject.lessons.length} тем пройдено</small></div></div></section>
    <div className="course-tabs"><button className="active">Учебные материалы</button><button>Контрольные мероприятия</button></div>
    <section className="materials"><div className="materials-title"><h2>УЧЕБНЫЕ МАТЕРИАЛЫ</h2><span>{subject.lessons.length} тем</span></div>
      {subject.lessons.map((l,i)=>{const complete=progress.completed.includes(lessonKey(subject.id,l.id));return <button className="material-row" key={l.id} onClick={()=>openLesson(subject,l)}><span className={complete?'num complete':'num'}>{complete?<Check size={16}/>:i+1}</span><div><strong>{l.title}</strong><small><Clock3 size={13}/>{l.duration} минут · теория, пример, практика и тест</small></div><ChevronRight size={18}/></button>})}
    </section>
    <section className="exam-card"><div><ListChecks/><span><strong>Итоговый контроль</strong><small>{subject.control}. Откроется после прохождения всех тем.</small></span></div><button disabled={done<subject.lessons.length}>Начать</button></section>
  </div>
}
...
function LessonScreen({subject,lesson,progress,note,updateNote,completeLesson,back}:{subject:Subject;lesson:Lesson;progress:Progress;note:string;updateNote:(v:string)=>void;completeLesson:()=>void;back:()=>void}){
  ...
  return <div className="lesson-page"><div className="lesson-toolbar"><button onClick={back}><ArrowLeft size={17}/>К темам</button><span>{subject.title}</span><div className={complete?'done-badge complete':'done-badge'}>{complete?<><CheckCircle2 size={15}/>Пройдено</>:<><Clock3 size={15}/>{lesson.duration} мин</>}</div></div>
    <article className="lecture">
      <header className={`lecture-cover ${subject.tone}`}><div className="cover-icon">{subject.icon}</div><p>{subject.title}</p><h1>{lesson.title}</h1><div className="cover-meta"><span><BookOpen size={16}/>Полная лекция</span><span><NotebookPen size={16}/>Конспект</span><span><Target size={16}/>Практика</span></div></header>
      <section className="learning-goal"><Target/><div><span>Цель занятия</span><p>{lesson.goal}</p></div></section>
      <div className="lecture-layout"><aside className="toc"><strong>На занятии</strong>{lesson.sections.map((s,i)=><a key={s.title} href={`#s${i}`}>{i+1}. {s.title.replace(/^\d+\.\s*/, '')}</a>)}<a href="#remember">Запомнить</a><a href="#practice">Практика</a><a href="#test">Тест</a></aside>
      <div className="lecture-body">
        ...
        <section id="remember" className="remember"><div className="section-label"><Brain size={18}/>Главное, что нужно унести с занятия</div>{lesson.keyFacts.map((f,i)=><div key={f} className="fact"><span>{i+1}</span><p>{f}</p></div>)}</section>
        <section className="example"><div className="section-label"><Sparkles size={18}/>Разобранный пример</div><p>{lesson.example}</p></section>
        <section id="practice" className="practice-block"><div className="section-label"><Target size={18}/>Самостоятельная работа</div><p>{lesson.practice}</p><div className="practice-tip">Не спеши смотреть ответ в другом источнике. Сначала реши сама и запиши ход мысли — именно это превращает чтение в обучение.</div></section>
        <section className="notes"><div className="section-label"><NotebookPen size={18}/>Мой конспект</div><textarea value={note} onChange={e=>updateNote(e.target.value)} placeholder="Запиши формулу, правило, вопрос преподавателю или объясни тему своими словами..."/><small>Сохраняется автоматически на этом устройстве.</small></section>
        <section id="test" className="test"><div className="test-head"><div><span>Тестовая часть</span><h2>Проверим, что осталось в голове</h2></div><div>{lesson.quiz.length} вопроса</div></div>
          ...
          {!checked?<button className="test-submit" disabled={!allAnswered} onClick={()=>setChecked(true)}>Проверить ответы</button>:<div className="test-result"><div><strong>{score}/{lesson.quiz.length}</strong><span>{score===lesson.quiz.length?'Отлично. Тема усвоена.':'Есть что повторить — перечитай объяснения выше.'}</span></div><button onClick={()=>{setAnswers({});setChecked(false)}}>Пройти ещё раз</button></div>}
        </section>
        <button className={complete?'complete-btn finished':'complete-btn'} onClick={completeLesson} disabled={complete}>{complete?<><CheckCircle2/>Занятие завершено</>:<><CheckCircle2/>Завершить занятие · +50 XP</>}</button>
      </div></div>
    </article>
  </div>
}
...
function PracticeScreen({openSubject}:{openSubject:(s:Subject)=>void}){
  ...
  return <div className="page"><div className="page-title"><div><p className="overline">Тренажёр</p><h1>Практика</h1><p>Отдельный режим, когда хочется не читать, а думать и решать.</p></div><Brain size={42}/></div>
...
function ProgressScreen({progress,totalLessons}:{progress:Progress;totalLessons:number}){
  ...
  const courseStats=subjects.map(s=>({s,done:s.lessons.filter(l=>progress.completed.includes(lessonKey(s.id,l.id))).length}))
  return <div className="page"><div className="page-title"><div><p className="overline">Статистика</p><h1>Мой прогресс</h1><p>Не просто «сколько нажато», а сколько материала действительно пройдено.</p></div><BarChart3 size={42}/></div>
    <div className="metric-grid"><Metric value={`${p}%`} label="общий прогресс" icon={<Target/>}/><Metric value={String(progress.completed.length)} label="занятий пройдено" icon={<CheckCircle2/>}/><Metric value={`${progress.minutes}`} label="минут обучения" icon={<Clock3/>}/><Metric value={`${progress.xp}`} label="XP заработано" icon={<Trophy/>}/></div>
    <section className="progress-panel"><h2>По предметам</h2>{courseStats.map(({s,done})=>{const pc=Math.round(done/s.lessons.length*100);return <div className="course-stat" key={s.id}><span className={`course-dot ${s.tone}`}>{s.icon}</span><div><strong>{s.title}</strong><i><em style={{width:`${pc}%`}}/></i></div><b>{done}/{s.lessons.length}</b></div>})}</section>
  </div>
}
